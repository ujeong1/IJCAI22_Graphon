...
def prepare_dataset_x(dataset):
    if dataset[0].x is None:
        ...
        if max_degree < 2000:
            ...
            for data in dataset:
                degs = degree(data.edge_index[0], dtype=torch.long)
                data.x = ((degs - mean) / std).view(-1,
                                                    1)  # z-score: how many standard deviations you are away from the mean. If a z-score is equal to 0, it is on the mean
    return dataset
...
if __name__ == '__main__':
    ...
    if gmixup == True:
        ...
        # generated graphs are 10*20*N (N = number of classes)
        # num_sample = int( train_nums * aug_ratio / aug_num )
        # num_sample = int(len(dataset)*0.2*0.1) #20% of original training dataset
        num_sample = num_class * 5
    ...
    with open("result_mix.txt", "a") as f:
